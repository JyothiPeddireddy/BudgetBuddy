...
export default function Reports() {
  ...
  /*
   * Export PDF / Excel
   * CHANGED: when "All time" is selected, sends all_time=true instead of
   * silently falling back to the current real month. Backend now handles
   * this by aggregating every transaction on file.
   */
  ...
  return (
    <div className="max-w-6xl mx-auto px-8 py-10 space-y-6">
      ...
      <div className="flex items-start justify-between gap-6 flex-wrap">
        <div>
          <h1 className="font-display text-2xl text-ink">Reports & Export</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <MonthYearPicker selected={viewPeriod} onSelect={setViewPeriod} />
        </div>
      </div>
      ...
      <div className="card p-6">
        <div className="flex items-center justify-between gap-6 flex-wrap">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="font-display text-lg text-ink">Export Report</h2>
...
function MonthYearPicker({ selected, onSelect }) {
  ...
  useEffect(() => { if (selected?.year) setViewYear(selected.year); }, [selected?.year]);
  ...
  const now = new Date();
  const isFutureMonth = (month) => viewYear === now.getFullYear() && month > now.getMonth() + 1;
      ...
      {open && (
        <div className="absolute right-0 mt-2 w-64 bg-white border border-slate-200 rounded-xl shadow-lg z-50 p-3">
          ...
          <div className="flex items-center justify-between px-1 mb-2">
            ...
            <button type="button" onClick={() => setViewYear((y) => y + 1)} disabled={isFutureYear} className="p-1 hover:bg-slate-50 rounded disabled:opacity-30">
            ...
            {MONTH_NAMES.map((month, index) => {
              ...
              return (
                <button
                  ...
                  className={`px-2 py-2 rounded-lg text-xs font-semibold transition-colors ${isSelected ? "bg-emerald text-white" : disabled ? "text-slate-300 cursor-not-allowed" : "text-ink hover:bg-slate-50"}`}
                >
                  {month.slice(0, 3)}
                </button>
              );
...
function StatCard({ label, value, pct, icon: Icon, color, invert = false }) {
  ...
  return (
    <div className="card p-5">
      ...
      <p className={`font-mono text-2xl font-bold ${styles.text}`}>{value}</p>
      {pct !== null && (
        <p className={`text-xs font-semibold mt-1.5 ${good ? "text-emerald" : "text-coral"}`}>
