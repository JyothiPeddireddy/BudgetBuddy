...
def add_expense(
    ...
    if budget:
        ...
        pct_used = (
            (total_spent / limit) * 100
            if limit > 0
            else 0
        )

        # -----------------------------------------------------
        # 25% - Early budget warning
        # -----------------------------------------------------
        if pct_used >= 25:
            message = (
                f"You've reached 25% of your "
                f"{expense.category} budget for {month_year}"
            )
        ...
        # -----------------------------------------------------
        # 50% - Half of budget used
        # -----------------------------------------------------
        if pct_used >= 50:
            message = (
                f"You've reached 50% of your "
                f"{expense.category} budget for {month_year}"
            )
        ...
        # -----------------------------------------------------
        # 75% - Close to budget limit
        # -----------------------------------------------------
        if pct_used >= 75:
            message = (
                f"You're close to your "
                f"{expense.category} budget limit for {month_year} "
                f"(75% used)"
            )
    ...
    # IMPORTANT:
    # Return the created expense so FastAPI can serialize it
    # using ExpenseOut.
    return expense
...
def get_dashboard(
    ...
    # Only apply date filtering if the user picked
