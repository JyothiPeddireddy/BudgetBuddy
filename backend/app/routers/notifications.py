...
from app.crud.notification import create_notification, get_notifications_by_user, mark_notification_read
...
def generate_monthly_report(db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    ...
    message = (
        ...
        f"Income {float(total_income):.2f}, Expenses {float(total_expenses):.2f}, Balance {balance:.2f}"
    )
